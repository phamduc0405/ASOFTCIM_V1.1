...
namespace ASOFTCIM
{
    ...
    {
        public void SendS8F4( List<INQUIRYFORM> inquiry, string ACK)
        {
            try
            {
                ...
                packet.addItem(DataType.List, inquiry.Count);
                foreach (var item in inquiry)
                {
                    packet.addItem(DataType.List, 2);
                    {
                        packet.addItem(DataType.Ascii, item.DATA_TYPE);
                        packet.addItem(DataType.List, item.ITEMs.Count);
                        foreach (var a in item.ITEMs)
                        {
                            packet.addItem(DataType.List, 3);
                            {
                                packet.addItem(DataType.Ascii, a.ITEMNAME);
                                packet.addItem(DataType.Ascii, a.ITEMVALUE);
