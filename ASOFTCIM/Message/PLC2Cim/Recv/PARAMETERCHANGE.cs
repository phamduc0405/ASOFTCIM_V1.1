using A_SOFT.CMM.INIT;
using A_SOFT.Ctl.Mitsu.Model;
using A_SOFT.PLC;
using ASOFTCIM.Data;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Reflection;
using System.Text;
using System.Threading.Tasks;

namespace ASOFTCIM.Message.PLC2Cim.Recv
{
    public class PARAMETERCHANGE
    {
        public void Excute(ACIM eq, object body)
        {

            try
            {
                BitModel bit = (BitModel)body;
                //List<WordModel> word = eq.PLCH.Words.Where(x => x.Area == bit.Comment).ToList();
                List<IWordModel> word = bit.LstWord;
                PPIDINFOR ppidinfor = new PPIDINFOR();
                //ppidinfor.MODE = word.FirstOrDefault(x => x.Item == "PPID").GetValue(eq.PLC);
                //ppidinfor.COMMANDCODEs = ;
                COMMANDCODE commandcode = new COMMANDCODE();
                List<PPIDModel> ppidList = new List<PPIDModel>(); 
                ppidList = eq.PLCH.ListPPID.ToList();

                for (int i = 0; i < ppidList.Count; i++)
                {
                    commandcode.PARAMs[i].PARAMNAME = ppidList[i].Item.ToString();
                    commandcode.PARAMs[i].PARAMVALUE = ppidList[i].GetValue(eq.PLC);
                    ppidinfor.COMMANDCODEs.Add(commandcode);
                }    

                eq.SendS7F107(ppidinfor);
                bit.SetPCValue = true;
            }
            catch (Exception ex)
            {
                var debug = string.Format("Class:{0} Method:{1} exception occurred. Message is <{2}>.", this.GetType().Name, MethodBase.GetCurrentMethod().Name, ex.Message);
                LogTxt.Add(LogTxt.Type.Exception, debug);
            }

        }
    }
}
